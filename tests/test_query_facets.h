...
static void runQueryFacetTests() {
    ...
    check(counts.folders == 1, "77.2 folder count");
    check(counts.images == 1, "77.3 image count");
    check(counts.videos == 1, "77.4 video count");
    check(counts.audio == 2, "77.5 audio includes matching symlink");
    check(counts.archives == 1, "77.6 archive count");
    check(me::file_category::matches(FileCategory::Files, 5, "asset-tool.app"),
          "77.7 app bundle is a file");
    check(!me::file_category::matches(FileCategory::Files, 2, "asset-folder"),
          "77.8 directory is not a file");
}
