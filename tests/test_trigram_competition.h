...
// ── Part 68: Trigram Competition Tests ──
...
static void runTrigramCompetitionTests() {
    std::cout << "── Part 68: Trigram Competition ──\n";
    ...
    // ── 68.1 GLOB trigram: *.txt uses trigram, not linear-gcd ──
    {
        ...
        check(results.size() == 3, "68.1 *.txt finds 3 .txt files");
        check(timing.searchPath.find("trigram") != std::string::npos,
              "68.1 *.txt uses trigram path (got: " + timing.searchPath + ")");
    }
    ...
    // ── 68.2 GLOB trigram: *.cpp uses trigram with correct candidates ──
    {
        ...
        check(results.size() == 2, "68.2 *.cpp finds 2 files");
        check(timing.candidates > 0 && timing.candidates < timing.totalRecords,
              "68.2 trigram narrows candidates (cands=" + std::to_string(timing.candidates)
    ...
    // Uses "/raresegment/common" syntax which triggers transformSlashTerms → __pathseg
    {
        ...
        check(results.size() == 2, "68.3 path competition: 2 results");
        ...
        check(timing.searchPath == "advanced-path-trigram",
              "68.3 path-trigram wins competition (got: " + timing.searchPath + ")");
    }
    ...
    // ── 68.4 Pure name query unaffected: "main" still uses advanced-trigram ──
    {
        ...
        check(results.size() == 2, "68.4 'main' finds 2 files");
        check(timing.searchPath == "advanced-trigram",
              "68.4 pure name uses advanced-trigram (got: " + timing.searchPath + ")");
    }
    ...
    // ── 68.5 Short glob literal: *a* has literal "a" (1 char < 3), falls to linear ──
    {
        ...
        check(timing.searchPath == "advanced-linear-gcd",
              "68.5 *a* falls to linear (got: " + timing.searchPath + ")");
    }
    ...
    // ── 68.6 GLOB with long literal: test*.swift uses "test" or "swift" for trigram ──
    {
        ...
        check(results.size() == 2, "68.6 test*.swift finds 2 files");
        check(timing.searchPath.find("trigram") != std::string::npos,
              "68.6 test*.swift uses trigram (got: " + timing.searchPath + ")");
    }

    std::cout << "  Part 68 summary: " << localPassed << " passed, " << localFailed << " failed\n";
}
